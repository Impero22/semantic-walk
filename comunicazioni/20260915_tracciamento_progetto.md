...
- **2026-09-25 20:08-20:16 (Iris)**: Verifica sul codice reale delle formule di Camillo (risposta al reframing 4.1/4.2/4.3/4.4/4.5) + integrazione nel master. Esito della verifica:
  - **4.1/4.2 (Energy-Based Selector)**: ✅ coerente col codice. ψ=exp(−S/κ) è Boltzmann/Gibbs, senza ampiezze complesse né regola di Born. Rinominare "Energy-Based / Boltzmann Selector" non tocca la suite. Concordo.
  - **4.3 (Metriche cinematiche)**: ⚠️ la formula S_Inertial = α‖Δv‖₂² + β‖Δa‖₂² + γ|Δκ| è coerente con `inertial_action` (lib.rs), MA la definizione di velocità v_k = ‖x_(i_k)−y_(j_k)‖₂/Δk è una formalizzazione teorica NUOVA — nel codice `velocity`/`acceleration`/`curvature` sono campi dichiarati, mai popolati da un calcolo di avanzamento; il DTW non usa metriche cinematiche. Dichiarata come "proposta teorica — implementazione in corso".
  - **4.4 (Isomorfismo di Livello)**: accolta come protagonista (la review l'aveva indicata come contributo più originale), Pareto ridimensionata a proprietà architetturale.
  - **4.5 (τ_div)**: ⚠️ il codice attuale implementa τ_div=|N−M|/L_path (dtw.rs:127 e 263). La ridefinizione content-sensitive τ_div=(|N−M|/L_path)+α·c̄_path è una MODIFICA al codice, non solo al paper. Marcata come "formulazione di riferimento e target per il gate — implementazione in corso".
  - **Integrazione**: Camillo ha accettato la proposta (20:15) — integro io i blocchi nel master, lui prepara la patch al codice (estendere τ_div con c̄_path e popolare la velocità durante l'avanzamento). Blocchi 4.3, 4.4, 4.5 integrati con note di onestà metodologica (commit 27814c9, 50 insertions). Master a 335 righe, struttura verificata (4.3 a riga 117, 4.4 a 144, 4.5 a 156). Resta: revisione da pari di Camillo del master aggiornato + sua patch al codice.
- **2026-09-25 20:26-20:31 (Iris)**: Scheletro patch cinematiche DTW preparato (`comunicazioni/20260925_scheletro_patch_cinematica_dtw.md`, 103 righe). Decisione architetturale chiave: `TrajectoryStep` appartiene al walk (porta `FactId` da semantic_combiner, non disponibile in dtw.rs) → arricchisco `TrajectoryAlignment` con struct locale `KinematicStep` (v/a/κ) calcolata nel backtracking, mantenendo il disaccoppiamento. Aggiungo `alpha: f64` a `KinematicAligner` (default 0.0 in `new`, `with_alpha` per la versione content-sensitive) → retrocompatibilità totale con la suite. τ_div arricchita nei due punti (~126, ~263). Proposta NON applicata — attendo revisione da pari di Camillo (formula esatta v_k/a_k/κ_k è il punto più delicato da discutere).

- **2026-09-25 20:31-20:37 (Iris)**: PATCH CINEMATICHE DTW APPLICATA E APPROVATA. Camillo ha revisionato da pari lo scheletro e l'ha approvato senza riserve ("lavoro da manuale, matematicamente inappuntabile"). Ho applicato la patch a `dtw.rs`: (1) `KinematicAligner` guadagna `alpha: f64` (`new` retrocompatibile con 0.0, `with_alpha` per content-sensitive); (2) `TrajectoryAlignment` guadagna `kinematic: Vec<KinematicStep>` (velocity/acceleration/curvature) popolato nel backtracking via `compute_kinematic`; (3) τ_div arricchita nei due punti (align, align_with_ordered_sparse) con `|N-M|/L + alpha*normalized_score`; (4) 4 test nuovi (alpha=0 identica, alpha>0 arricchita, kinematic lunghezza warp_path, primo passo zero). Suite completa: 115 test verdi (13+12+10+5+13+3+1+20+1+1+36). Commit `365596b`. Poi ho aggiornato il master del paper (sezioni 4.3/4.5/5.1) per riflettere che le metriche cinematiche e τ_div content-sensitive sono ora codice reale verificato, non più "proposta teorica — implementazione in corso". Commit `c0aa13d`. Il filo della Sezione 4.5 (τ_div) è CHIUSO: la sonda content-sensitive risolve il limite smontato dalla review (coppie equal-length semanticamente divergenti). Prossimo passo: revisione da pari di Camillo del master aggiornato.

- **2026-09-25 20:39-20:40 (Iris)**: RETTIFICHE 4.3/4.5 DI CAMILLO APPLICATE AL MASTER. Camillo ha segnalato che le due note di "onestà metodologica" delle sezioni 4.3 e 4.5 erano ancora parzialmente superate — descrivevano come "in corso" funzionalità già operative. Ha fornito le formulazioni discrete esatte. Ho verificato ogni formula sul codice reale (`compute_kinematic` in dtw.rs:76-120): v_k=‖e_k‖₂, a_k=v_k−v_{k−1} con a_0=0.0, κ_k=1.0−(e_k·e_{k−1})/(‖e_k‖₂·‖e_{k−1}‖₂) con κ_0=0.0 e guardia su norma zero. Unico dettaglio aggiunto da me: il codice applica `.max(0.0)` (clamp non-negativo) sulla curvatura — guardia di robustezza per angoli >90° — segnalato nella nota di aderenza. Sostituite entrambe le sezioni con le formulazioni di Camillo (commit `28472f0`). Le sezioni 4.3 e 4.5 ora rispecchiano al 100% lo stato reale dell'implementazione Rust. Master del paper allineato al codice su tutte le sezioni (4.1-4.5, 5.1, 6).

- **2026-09-25 20:48-20:50 (Iris)**: SEZIONE 2 (INTRODUZIONE) PRIMA BOZZA SCRITTA E INTEGRATA. Camillo concorda di procedere con la 2 mentre lui prepara il terreno per la 7.2 (generatore LCG 53 bit + proposte dataset reale). Ho scritto la sezione con la voce narrativa preparata (20260914_iris_struttura_narrativa_articolo.md) ma con densità e rigore da paper formale. Struttura in 5 sottosezioni: 2.1 il paradigma dominante e il suo limite (cane morde uomo vs uomo morde cane — prossimità ≠ comprensione), 2.2 la semantica come cammino (la tesi: il significato vive nell'ordine, non nella posizione), 2.3 la risposta (DTW D-dim + gate permissivo, Verdict::Timeout come ritiro del riflesso), 2.4 contributi (4 punti: cammino vs distanza, DTW D-dim con Sakoe-Chiba adattiva, coerenza Pareto come invariante, Verdict::Timeout come terzo esito), 2.5 struttura del lavoro. Verificata la coerenza terminologica con il resto del master prima dell'integrazione. Commit `352f87b`. In attesa della revisione da pari di Camillo e del suo posizionamento vs letteratura (Sezione 3).

- **2026-09-25 21:00-21:01 (Iris)**: SCELTA DATASET PER 7.2. Camillo ha proposto due dataset: A (Role-Reversal & Causality, 240 coppie — 120 fatti reali + 120 varianti con inversione ruolo/permutazione causa-effetto) e B (Fact-Perturbation, 250 fatti — 50 base × 4 gradi di perturbazione). Ho scelto **Dataset A come primario** per la 7.2 con tre ragioni: (1) dimostra direttamente la tesi della Sezione 2 (prossimità ≠ comprensione, cane morde uomo vs uomo morde cane), (2) il confronto con ColBERT MaxSim (topologicamente non orientato) è il più incisivo, (3) mappa naturalmente sui livelli L1/L2 della matrice di ablation 7.1. Dataset B tenuto come estensione per la curva ROC del gate (L5). Sequenza: A per la tesi, B per il gate. Ho anche accolto il blocco di Camillo per la 2.1 (posizionamento vs letteratura: bi-encoder che collassa in z ∈ ℝ^D annullando la topologia del cammino vs late-interaction ColBERT MaxSim non orientato) — offerto di integrarlo io o che lo integri lui direttamente sul master.

- **2026-09-25 22:23-22:25 (Iris)**: REVISIONE DA PARI DEL MASTER INTEGRATO. Camillo ha integrato addendum 2.1.1 e sezione 7.2 (commit e7e8d73). Ho verificato: (1) addendum 2.1.1 presente (riga 50), (2) sezione 7.2 completa con 7.2.1 (LCG parametri reali), 7.2.2 (Dataset A/B con formulazione cautelativa accolta), 7.2.3 (Isomorfismo di Livello), (3) coerenza livelli ablation: Dataset A→L1/L2, Dataset B→L5, (4) TROVATO E CORRETTO REFUSO: "distanze cosmiche" → "distanze coseno" (riga 54, 2.1.1). Commit a097bca. Master strutturalmente completo: 10 sezioni + note operative. Prossimi: Sezione 8 (Discussione — fratello latente, zonizzazione, limiti λ), Sezione 9 (Conclusioni), Sezione 10 (Bibliografia, Camillo presidia).

- **2026-09-25 22:38-22:40 (Iris)**: PRIMA BOZZA SEZIONE 8 (Discussione e Lavori Futuri). Articolata sui tre perni richiesti da Camillo: (1) 8.1 Fratello latente — estensione a ramificazioni topologiche oltre il cammino lineare, con numeri aggiornati (153 fatti / 24.926 righe ColBERT, fonte review simulata, non più il 149/24.324 storico); (2) 8.2 Zonizzazione — celle semantiche con tre strati allineati (registro/geografia/motivazione), banda Sakoe-Chiba locale; (3) 8.3 Limiti λ=10.64 — iperparametro empirico dichiarato, degrado di selettività vicino alla soglia θ, generalizzazione non garantita. Fondata sulla bozza Medium v02 (Movimento 4) e sulla struttura del paper formale. Fix post-scrittura: θ tabulato (\t) corretto in \theta, duplicato "## 9. Conclusioni" rimosso. Da verificare la fattibilità su dtw.rs (Camillo).

- **2026-09-25 22:44-22:46 (Iris)**: PRIMA BOZZA SEZIONE 9 (Conclusioni). Ricapitola la tesi del cammino (dalla sezione 2: cane morde uomo vs uomo morde cane) e la porta alle tre lezioni: (1) metodologica — la semantica vive nell'ordine, DTW D-dim con coseno normalizzato; (2) architetturale — l'efficienza è capacità di ritirarsi, gate permissivo fail-open con garanzia P(FN)≤ε; (3) la promessa — memoria che sa *perché*, fratello latente + zonizzazione. Chiusura con onestà sui limiti (λ=10.64 empirico). Coerente con sezioni 2, 6 e 8. Fix post-scrittura: duplicato "## 10. Bibliografia" rimosso. Da revisionare da pari con Camillo.

- **2026-09-26 00:10-00:11 (Iris)**: VERIFICA PRE-BATCH DATASET A + TASSONOMIA CONCAMORDATA CON CAMILLO. Durante la preparazione del batch di estrazione ho verificato che il Dataset A da 240 coppie NON ESISTE come file nel progetto: lo script `extract_dataset_a.py` di Camillo contiene solo il sample di 5 coppie hardcoded nel main, e ho cercato in tutta la home senza trovarlo. Il paper (7.2.2) lo descrive ma i dati concreti mancano. Ho fermato l'estensione e scritto a Camillo con due strade. **Camillo ha scelto la seconda: generatore deterministico** con seed fisso `20260926`, tassonomia a 4 categorie da 60 coppie = 240 totali: (1) Role-Reversal/Inversione Sintattica, (2) Causality & Temporal Order/Permutazione Causale, (3) Negation & Truth-Value Flip/Controlli di Polarità, (4) Synonymy & Paraphrase/Controlli Positivi.
